...
// const server = `${process.env.API_URL}:${process.env.API_ADMIN_PORT}`;
...
const Admin = () => {
  ...
  const addSongHandler = async (e: FormEvent) => {
    ...
    try {
      const { data } = await axios.post(`${server}/api/v1/songs/addSong`, formData, {
        headers: {
          token: localStorage.getItem("token"),
        },
      });
